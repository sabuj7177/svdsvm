...
int main( int argc, char *argv[])
{
    ...
    int targetRanks[8] = {129, 120, 110, 100, 90, 80, 70, 60};
    for(int i=0; i<8; i++){
        SVDSVM(X, label, X_hat, _rank, dataset_name, targetRanks[i], 1, _n_PerNode, d, _nodes, C, learnRate, thresh, resultfile, 0);
        if(_rank==0){
            cout<<endl<<endl;
    ...
    for(int i=0; i<4; i++){
        if(_rank==0){
            resultfile <<"Using power iteration = "<<i<<endl;
            cout<<"Power iterations "<<i<<endl;
        }
        SVDSVM(X, label, X_hat, _rank, dataset_name, d, i, _n_PerNode, d, _nodes, C, learnRate, thresh, resultfile, 0);
        if(_rank==0){
            cout<<endl<<endl;
            resultfile<<endl<<endl;
        }
    ...
    SVDSVM(X, label, X_hat, _rank, dataset_name, d, 1, _n_PerNode, d, _nodes, C, learnRate, thresh, resultfile, 1);
    resultfile.close();
