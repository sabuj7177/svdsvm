...
void test(const arma:: mat& R, const arma:: vec& alpha_hat, const string dataset_name,  ofstream& resultfile){
    ...
    mat weight_mat = R.t() * alpha_hat(span(0,d-1));
...
int main( int argc, char *argv[])
{
    ...
    float summed_qr_time = 0, summed_da_time = 0, summed_total_time = 0;
    if(_rank==0){
        resultfile <<"In QRSVM"<<endl;
        cout<< "In QRSVM" <<endl;
        if(cost_function==1){
            cout<<"Computing cost function"<<endl;
            resultfile <<"Computing cost function"<<endl;
        }
    }
    ...
    for(int k=0;k<repeat_computation;k++){
        ...
        summed_qr_time += get_time_duration(startTotalQR);
        }
        ...
        if(cost_function==1){
                if(_rank==0){
                    weightmat = rF.t() * alpha_hat(span(0,d-1));
                }
        ...
        if(_rank==0){
            ...
            time_toc("Total dual ascent time ", startDA );
            time_toc("Total time ", start_Traintime );
            summed_da_time += get_time_duration(startDA);
            summed_total_time += get_time_duration(start_Traintime);
            resultfile <<"Total iteration = "<<count<<endl;
            resultfile <<"Communication in QR = "<<qr_comm<<endl;
            resultfile <<"Communication in DA step = "<<da_comm<<endl;
            resultfile <<"Total Communication = "<<qr_comm+da_comm<<endl;
            resultfile <<"Per iteration time = "<<round_to_four_decimal(get_time_duration(iteration_start)/(count*1.0))<<endl;
            test(rF, alpha_hat, dataset_name, resultfile);
        }
    ...
    if(_rank==0){
        resultfile <<"QR Time = "<<round_to_four_decimal(summed_qr_time/(repeat_computation*1.0))<<endl;
